...
###   bitphantomcore.org and bitphantom.org and compares them.
...
WORKINGDIR="/tmp/bitphantom_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitphantomcore.org"
HOST2="https://bitphantom.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitphantom-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitphantom-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "bitphantom.org failed to provide signature file, but bitphantomcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitphantom.org and bitphantomcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the bitphantom Core binary release signing key installed?"
   fi
