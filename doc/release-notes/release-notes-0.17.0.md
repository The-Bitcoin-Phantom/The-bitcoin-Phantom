bitphantom Core version 0.17.0 is now available from:

  <https://bitphantomcore.org/bin/bitphantom-core-0.17.0/>
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the bitphantom.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
[BIP 174 PSBT](https://github.com/bitphantom/bips/blob/master/bip-0174.mediawiki) is an interchange format for bitphantom transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://bitphantomtalk.org/?topic=279249) transactions.
...
- #11962 `0a01843` add seed.bitphantom.sprovoost.nl to DNS seeds (Sjors)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI bitphantom:// (krab)
...
- #13528 `b877c39` Move bitphantomGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libbitphantom-util to libbitphantom-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for bitphantom-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling bitphantom-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use bitphantomTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to bitphantom.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use bitphantomTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use bitphantomTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use bitphantomTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for bitphantomTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_bitphantom` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.bitphantom is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_bitphantom` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in bitphantomd systemd service file (Flowdalic)
- #12652 `55f490a` bitphantom-cli: Provide a better error message when bitphantomd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to bitphantomd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for bitphantom address in translations (kaplanmaxe)
...
- #13603 `453ae5e` bitphantom-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix bitphantom-cli --version (Empact)
...
- #12877 `2b54155` Use bitphantomd in Tor documentation (knoxcard)
...
- #13346 `60f0358` update bitphantom-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust bitphantomcore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed bitphantom-cli -help output for help2man (hebasto)
