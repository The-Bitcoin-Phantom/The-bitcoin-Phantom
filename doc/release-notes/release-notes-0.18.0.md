bitphantom Core version 0.18.0 is now available from:

  <https://bitphantomcore.org/bin/bitphantom-core-0.18.0/>
...
`/Applications/bitphantom-Qt` (on Mac) or `bitphantomd`/`bitphantom-qt` (on
Linux).
...
bitphantom Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use bitphantom Core on unsupported systems.
...
10.10. Additionally, bitphantom Core does not yet change appearance when
...
`contrib/init/bitphantomd.service`, it has been changed to use
`/var/lib/bitphantomd` as the data directory instead of
`~bitphantom/.bitphantom`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bitphantomd` will
exist has enough space (check using `df -h /var/lib/bitphantomd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bitphantom/bitphantom/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitphantomcore.org/en/contact/) the bitphantom Core
...
The systemd init file (`contrib/init/bitphantomd.service`) has been changed
to use `/var/lib/bitphantomd` as the data directory instead of
`~bitphantom/.bitphantom`. This change makes bitphantom Core more consistent with
...
`datadir` option in `/etc/bitphantom/bitphantom.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/bitphantom/bitphantom.conf`.


Documentation
-------------

- A new short [document](https://github.com/bitphantom/bitphantom/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bitphantom/bitphantom/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bitphantom/bitphantom/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bitphantom/bitphantom/blob/master/doc/bitphantom-conf.md)
  about the `bitphantom.conf` file describes how to use it to configure
  bitphantom Core.

- A new document introduces bitphantom Core's BIP174 [Partially-Signed
  bitphantom Transactions
  (PSBT)](https://github.com/bitphantom/bitphantom/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bitphantom/bitphantom/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent bitphantom-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  bitphantom Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bitphantom/bitphantom/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bitphantom/bitphantom/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when bitphantom Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of bitphantom Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bitphantom/bitphantom/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bitphantom/bitphantom/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bitphantomd with the `-deprecatedrpc=generate`
...
- A new `bitphantom-wallet` tool is now distributed alongside bitphantom
...
This section describes planned changes to bitphantom Core that may affect
other bitphantom software and services.

- Since version 0.16.0, bitphantom Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with bitphantom Core 0.20 (expected about
  a year after 0.18), bitphantom Core will default to native segwit
  ...
  to bech32 addresses, and if the bitphantom Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in bitphantom Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any bitphantom Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, bitphantom Core
...
- A sub-project of bitphantom Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with bitphantom Core.  See their [project
  page](https://github.com/bitphantom-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to bitphantom Core's own implementation, although entropy
  gathered by bitphantom Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves bitphantom Core a
...
- On macOS, bitphantom Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `bitphantom_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbitphantom_server.a` (jnewbery)
...
- #13926 [Tools] bitphantom-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bitphantomd (MarcoFalke)
...
- #14521 qt, docs: Fix `bitphantom-qt -version` output formatting (hebasto)
...
- #11625 Add bitphantomApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbitphantom_server` (laanwj)
...
- #15154 Configure: bitphantom-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bitphantom-wallet (Sjors)
...
- #11526 Visual Studio build configuration for bitphantom Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbitphantom_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in bitphantom-util-test.py (isghe)
...
- #12775 Integration of property based testing into bitphantom Core (Christewart)
...
- #12246 Bugfix: Only run bitphantom-tx tests when bitphantom-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bitphantom` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bitphantom` (MarcoFalke)
...
- #14186 bitphantom-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bitphantom.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bitphantom.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bitphantom-conf.md (hebasto)
...
- #15354 Add missing `bitphantom-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bitphantomconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bitphantom folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/bitphantom/).
