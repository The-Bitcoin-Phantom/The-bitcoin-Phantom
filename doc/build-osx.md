...
1. Clone the bitphantom Core source code:
    ```shell
    git clone https://github.com/bitphantom/bitphantom
    cd bitphantom
    ```

2.  Build bitphantom Core:

    Configure and build the headless bitphantom Core binaries as well as the GUI (if Qt is found).
...
## `disable-wallet` mode
When the intention is to run only a P2P node without a wallet, bitphantom Core may be
...
```shell
mkdir -p "/Users/${USER}/Library/Application Support/bitphantom"

touch "/Users/${USER}/Library/Application Support/bitphantom/bitphantom.conf"

chmod 600 "/Users/${USER}/Library/Application Support/bitphantom/bitphantom.conf"
```

The first time you run bitphantomd, it will start downloading the blockchain. This process could
...
processors only.
* Building with downloaded Qt binaries is not officially supported. See the notes in [#7714](https://github.com/bitphantom/bitphantom/issues/7714).
...
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  script that is also included there. Detached signatures are available from this [repository](https://github.com/bitphantom-core/bitphantom-detached-sigs).
