    ...
    if(role == Qt::DisplayRole || role == Qt::EditRole)
    {
        ...
        case Amount:
            if (rec->recipient.amount == 0 && role == Qt::DisplayRole)
                ...
                return bitphantomUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount, false, bitphantomUnits::separatorNever);
            else
                return bitphantomUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), rec->recipient.amount);
        }
    }
...
{
    return (this->walletModel->getOptionsModel() != nullptr) ? tr("Requested") + " ("+bitphantomUnits::shortName(this->walletModel->getOptionsModel()->getDisplayUnit()) + ")" : "";
}
