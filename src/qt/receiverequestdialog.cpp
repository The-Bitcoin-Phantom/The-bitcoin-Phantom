...
#include <config/bitphantom-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::update()
{
    ...
    QString uri = GUIUtil::formatbitphantomURI(info);
    ...
    if(info.amount)
        html += "<b>"+tr("Amount")+"</b>: " + bitphantomUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount) + "<br>";
...
{
    GUIUtil::setClipboard(GUIUtil::formatbitphantomURI(info));
}
