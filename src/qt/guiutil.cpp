...
bool parsebitphantomURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!bitphantomUnits::parse(bitphantomUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatbitphantomURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("bitphantom:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(bitphantomUnits::format(bitphantomUnits::BTC, info.amount, false, bitphantomUnits::separatorNever));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", bitphantom_CONF_FILENAME));
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / strprintf("bitphantom-%s.desktop", chain);
}
...
bool GetStartOnSystemStartup()
{
    ...
    bool res = (findStartupItemInList(listSnapshot, loginItems, bitphantomAppUrl) != nullptr);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    CFArrayRef listSnapshot = LSSharedFileListCopySnapshot(loginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(listSnapshot, loginItems, bitphantomAppUrl);
