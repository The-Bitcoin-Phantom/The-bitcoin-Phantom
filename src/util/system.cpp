...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitphantom-tx using stdin
        std::string val;
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure bitphantom Core copyright is not removed by accident
    if (copyright_devs.find("bitphantom Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The bitphantom Core developers";
    }
