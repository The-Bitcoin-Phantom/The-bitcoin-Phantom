 ...
 */
static const char* bitphantom_PID_FILENAME = "bitphantomd.pid";
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", bitphantom_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitphantom/bitphantom>";
    const std::string URL_WEBSITE = "<https://bitphantomcore.org>";
...
/** Sanity checks
 *  Ensure that bitphantom is running in a usable environment with all
...
{
    // Make sure only a single bitphantom process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", bitphantom_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bitphantom is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitphantom is started while in a temporary directory.\n",
